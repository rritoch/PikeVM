...
array(mapping) query_translations()
{
  return ({ translations, null_translations, identity_translations });
}

array(string) query_restrictions() { return copy(restrictions); }

mapping defaults() { return _defaults; }

private string null_trans() {
    return "";
}
private string identity_trans(string str) {
   return "%^" +str + "%^";
}
...
"LS_HEADING" : "bold"
  ...
  translations = translations + _defaults;


  null_translations = map(translations, null_trans );
  identity_translations = map(translations, identity_trans);
}
...
void add_default_colour(string key, string value, int wiz_only)
{
  ...
  _defaults[upper_case(key)] = lower_case(value);
  if(wiz_only)
    restrictions += ({ upper_case(key) });
  resync();
}
...
void remove_default_colour(string key)
{
  ...
  map_delete(_defaults, upper_case(key));
  restrictions += ({ upper_case(key) });
  resync();
}