...
private int match_path(string pattern,string path)
{

    int pp = 0;
    int sp = 0;

    int psz = sizeof(pattern);
    int ssz = sizeof(path);
    ...
    while(pp < psz && sp < ssz) {
        if (pattern[pp] == '*') {
            ...
            if (pp < psz) {
                while(sp < ssz) {
                    if (match_path(pattern[pp..],path[sp..])) {
                        return 1;
                    }
                    if (path[sp] == '/') {
                        return 0;
            ...
            if (pattern[pp] != '?' && pattern[pp] != path[sp]) {
                return 0;
            }
            sp++;
            pp++;
    ...
    if (pp + 1 == psz && pattern[pp] == '*') {
        return 1;
    }
    return pp == psz && sp == ssz;
}
