...
#ifdef USE_LOCAL_LIBRARY_PATH
    string pike_library_path = dirname(__FILE__);
#else             
    string pike_library_path = STATIC_PIKE_LIBRARY_PATH;    
#endif
...
array(string) master_get_dir(string|void x) {
 if (kernel_registered) {
  if (stringp(x)) return kernel->_get_dir(x);
  return kernel->_get_dir();
 }
...
protected void create()
{
  ...
  //add_constant("load_module",master_load_module);
...
{
#ifdef DEBUG_INHERIT
 write("master()->handle_inherit(%O,%O,%O)\n",pname,current_file,handler);
#endif
			  ...
			  object|void current_handler)
{
  ...
  object o;
  if (kernel_registered) {
   o = kernel->find_object(oname,current_file);
...
program master_empty_program(int|void line, string|void file) {
 ...
 argc = query_num_arg();
 if (kernel_registered) {
  if (argc > 1) return kernel->___empty_program(line,file);
  if (argc == 1) return kernel->___empty_program(line);
  return kernel->___empty_program();
 }
...
class Describer
{
  ...
  string describe(mixed m, int maxlen) {
   //if (kernel_registered) return kernel->describe(m,maxlen);
    catch {
...
string intern_describe_module(object|program mod, array(object)|void ret_obj) {
...
string describe_module(object|program mod, array(object)|void ret_obj) {
 if (kernel_registered) return kernel->describe_module(mod,ret_obj);
 return intern_describe_module(mod,ret_obj);
}
