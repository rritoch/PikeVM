...
array(string) regexplode(string str, string pat) {
    array(mixed) tmp;
    tmp = reg_assoc(str, ({ pat }), ({ 1 }) );
    return tmp[0];
}

array(string) split( string str, string pattern ){
  int i, j;
  mixed x,r;
...
string insensitive_pattern(string pat)
{
  int i;
  int bracket;
  int patlen;
...
array(string) insensitive_regexp(array(string) arr, string pat, int flag)
{
