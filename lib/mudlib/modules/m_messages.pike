...
void set_msgs(string cls, array(string) msgs)
{
  if (!messages)
    messages = ([]);
  if(!msgs || !sizeof(msgs))
...
array handle_obs(array obs, string res, mapping has)
{
  ...
  else
  {
    ...
    if (res[<2..<1]=="a ")
    {
      ...
      {
        if(items[ob]>1)
          ret += ({ items[ob] + " " + load_object(M_GRAMMAR)->pluralize(SHORT(ob)) });
        else
          ret += ({ _a_short(ob) });
      }
    } else if (res[<4..<1] == "the ") {
      ...
      {
        if(items[ob]>1)
          ret += ({ "the " + items[ob] + " " + load_object(M_GRAMMAR)->pluralize(SHORT(ob)) });
        else
          ret += ({ _the_short(ob) });
      }
    } else if (res[<2..<1] == "A ") {
...
//Usually this routine is used through the higher level interfaces.
 string compose_message(object forwhom, string msg, array(object) who,
  mixed ... obs)
{
  mixed ob;
  array fmt;
  ...
  {
    c = fmt[i][1];
    if (sizeof(fmt[i]) > 2 && fmt[i][2] && fmt[i][2]<'a')
    {
      if (sizeof(fmt[i]) > 3 && fmt[i][3] && fmt[i][3] < 'a')
      {
        subj = fmt[i][2] - '0';
        num = fmt[i][3] - '0';
        str = fmt[i][4..<0];
...
/* hack for contractions */
        ...
        if (num >= sizeof(who) || who[num]!=forwhom)
          bit = load_object(M_GRAMMAR)->pluralize(str);
        else
          bit = str;
        break;
      case 'p':
...
//### Hack to avoid inheriting a mixin. Better one needed.
    if (fmt[i+1][0] == '.')
      res += load_object(M_GRAMMAR)->punctuate(bit) + fmt[i+1][1..];
    else
      res += bit + fmt[i+1];
    i+=2;
  }
...
//see: inform
 ...
 array(string) action(array(object) who, mixed msg, mixed ... obs)
{
  int i;
  array(string) res;
...
void inform(array(object) who, array(string) msgs, mixed others)
{
  int i;
  mapping done = ([]);
  ...
  if (arrayp(others))
    map_array(others - who, tell_from_outside, msgs[-1], MSG_INDENT);
  else if (others)
...
//some objects
 void simple_action(mixed msg, mixed ... obs)
{
  string us;
  string others;
  array(object) who;
...
//Generate and send a message that should only be seen by the person doing it
 void my_action(mixed msg, mixed ... obs)
{
  string us;
  array(object) who;
...
//Generate and send a message that should only be seen by others
 void other_action(mixed msg, mixed ... obs)
{
  string others;
  array(object) who;
...
//other objects)
 void targetted_action(mixed msg, object target, mixed ... obs)
{
