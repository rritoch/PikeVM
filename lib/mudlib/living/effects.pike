...
private array effects = ({}); // Queue of effects to call
 private int tag; // Tag for call_out
...
{
  array(mixed) callouts = call_out_info();
  mixed callout;
  foreach(callouts, callout)
  {
...
// array(int) ({ fixed, var}) : period = fixed + random(var)
...
// Return ({})
array(int) find_effect_indexes_matching(string name)
{
  array(int) res = ({});
  if(name)
    for(int i = 0; i<sizeof(effects); i++)
...
{
  array(int) matches = find_effect_indexes_matching(name);
  if(sizeof(matches))
  {
  	int idx;
    foreach(sort_array(matches, -1),idx)
...
void insert_effect_at(effect_class effect, int pos)
{
...
int insert_effect(effect_class effect)
{
  effect_class this_effect = effect_class();
...
void add_effect(string ob, mixed args, int repeats, mixed interval)
{
  ...
  if(undefinedp(repeats))
    repeats = obj->query_repeats();
  if(undefinedp(interval))
    interval = obj->query_interval();

  this_effect->effect_ob = ob;
  this_effect->name = obj->query_name();
