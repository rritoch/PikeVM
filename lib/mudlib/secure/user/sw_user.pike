...
void switch_body(string new_body_fname, int permanent);

void register_failure(string addr);

void modal_simple(function input_func, array(mixed) input_func_args, mixed prompt, int|void secure);

void set_privilege(mixed priv);	// from M_ACCESS
mixed unguarded(mixed priv, function code);

void clear_input_stack();


final mixed call(function f, mixed ... args) 
{
	return f(@args);
}

private final int do_su(string old_userid, string new_userid, string new_body)
{
    object body;
    string old_name;
    string new_name;
...
private final void confirm_valid_su(string old_userid,
string new_userid,
string new_body,
string arg)
{
    ...
    pwd = call_other(USER_D,"query_variable",new_userid,({ "password" }))[0];
...
final void switch_user(string str, string new_body)
{
    string linkpath;
...
return;
    ...
    {
        linkpath = LINK_PATH(new_userid) + __SAVE_EXTENSION__;

if( file_size(linkpath) <= 0 ) {
    write("No such user.\n");
    return;
}
    }
